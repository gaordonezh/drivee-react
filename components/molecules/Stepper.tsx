...
const Stepper = ({ steps }: { steps: Array<number> }) => {
  ...
  const dotClass = 'flex items-center justify-center w-10 h-10 rounded-full lg:h-12 lg:w-12 shrink-0';

  const active = 'bg-black text-white';
  const inactive = 'bg-white text-black';

  const activeContainer = 'bg-white';
  const inactiveContainer = 'bg-slate-200';
